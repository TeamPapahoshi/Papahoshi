...
void cStageSelect::Init()
{
	...
	for (int HorizonLoop = 0; HorizonLoop < MAX_STAGE / (MAX_STAGE / 2); HorizonLoop++)	//行のループ
	{
		for (int VarticalLoop = 0; VarticalLoop < MAX_STAGE / 2; VarticalLoop++, pStageSprite++, pStageClearSprite++) //列のループ
		{
			...
			//全初期化用
			//m_bClearFlag[VarticalLoop + HorizonLoop * MAX_STAGE / 2] = false;
		}
	}
...
void cStageSelect::SaveClearFlag(void)
{
	...
	//ファイルを新規作成して書き込み開始
	ofs.open("Test.bin", ios_base::out | ios_base::trunc | ios_base::binary);
	...
	{
		ofs.write((char*)&m_bClearFlag[SaveFlagLoop], sizeof(bool));
	}
...
{
	ifstream ifs("Test.bin", ios_base::in | ios_base::binary);	//ファイル読み込み用
	...
	{
		ifs.read((char*)&m_bClearFlag[LoadFlagLoop], sizeof(bool));
	}
}
...
//
//		ステージクリアフラグの設定関数
//		引数：int型 フラグを立てるステージの番号
//		戻り値：なし
//
//=======================================================================================
void cStageSelect::SetStageClearFlag(int num)
{
	m_bClearFlag[num] ^= 1;	//配列の先頭から引き渡し
}